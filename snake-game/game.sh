...
init_snake() {
    ...
    for ((i=0; i<length-1; i++)); do
        body="1$body"                       # РАЗОБРАТЬСЯ СО СТРОКОЙ
    done
...
move_snake() {
    ...
    if [ $newhead_r -ge $height ]; then
        newhead_r=0
    elif [ $newhead_r -lt 0 ]; then
        newhead_r=$((height-1))
    fi

    if [ $newhead_c -ge $width ]; then
        newhead_c=0
    elif [ $newhead_c -lt 0 ]; then
        newhead_c=$((width-1))
    fi
...
#попробовать написать змейку с нуля -- -- -- -- --- -- -- -- с использованием функции очистки экрана и перемещением курсора
